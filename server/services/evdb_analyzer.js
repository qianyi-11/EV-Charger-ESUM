...
/**
 * Estimate DIN pole/module count from bbox aspect ratio (portrait EVDB photos).
 * MCB: 1P ~0.15–0.33, 2P ~0.33–0.57, 3P ~0.57–0.88, 4P ~0.88+
 * RCCB: wider enclosures — 2P single-phase units often ratio 0.35–0.68 (e.g. 0.61).
 */
export function inferPoleCountFromBox(box, { component = 'generic' } = {}) {
  const ratio = boxAspectRatio(box);
  if (ratio == null) return null;

  if (component === 'rccb') {
    if (ratio < 0.36) return 1;
...
/** When spec is single-phase, avoid classifying borderline-wide boxes as 3P+. */
export function applyExpectedPhaseHint(poleCount, ratio, expectedSupplyPhase) {
  if (expectedSupplyPhase !== 1 || poleCount == null || ratio == null) return poleCount;
  if (poleCount >= 3 && ratio < 0.75) return 2;
  return poleCount;
}
...
function describePoleMismatch(component, poleCount, ratio, expectedPhase, inputVoltage) {
  ...
  return `${component} phase mismatch: ${supplyPhase} phase supply, spec required ${expectedPhase} phase.`;
}
...
export function analyzeEvdbCompliance({ detections, specsContext = {} }) {
  ...
  let mcbPoles = mcbDet ? inferPoleCountFromBox(mcbDet.box, { component: 'mcb' }) : null;
  let rccbPoles = rccbDet ? inferPoleCountFromBox(rccbDet.box, { component: 'rccb' }) : null;
  const mcbRatio = mcbDet ? boxAspectRatio(mcbDet.box) : null;
  const rccbRatio = rccbDet ? boxAspectRatio(rccbDet.box) : null;
  mcbPoles = applyExpectedPhaseHint(mcbPoles, mcbRatio, expectedPhase);
  rccbPoles = applyExpectedPhaseHint(rccbPoles, rccbRatio, expectedPhase);
